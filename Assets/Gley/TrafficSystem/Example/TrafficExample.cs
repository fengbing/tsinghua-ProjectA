    ...
    public class TrafficExample : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (Input.GetKeyDown(KeyCode.F))
            {
                ...
                if (_followVehicle)
                {
                    GameObject.Find("Main Camera").GetComponent<UrbanSystem.Internal.CameraFollow>().target = API.GetVehicleComponent(_vehicleToFollow).transform;
                    API.SetCamera(API.GetVehicleComponent(_vehicleToFollow).transform);
                }
                else
                {
                    GameObject.Find("Main Camera").GetComponent<UrbanSystem.Internal.CameraFollow>().target = _player;
