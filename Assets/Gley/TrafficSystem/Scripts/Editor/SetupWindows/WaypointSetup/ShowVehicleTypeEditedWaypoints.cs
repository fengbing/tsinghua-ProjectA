...
namespace Gley.TrafficSystem.Editor
{
    public class ShowVehicleTypeEditedWaypoints : ShowWaypointsTrafficBase
    {
        ...
        {
            base.TopPart();
            if (GUILayout.Button("Delete all vehicle edited waypoints"))
            {
                if (EditorUtility.DisplayDialog("Delete All Waypoints", "Are you sure you want to delete all vehicle edited waypoints?", "Yes", "No"))
                {
                    foreach (var waypoint in _waypointsOfInterest)
                    {
                        waypoint.carsLocked = false;
