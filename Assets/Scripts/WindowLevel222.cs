...
/// <summary>
/// Level 2 场景的 Esc 暂停菜单控制器。
/// 流程：按 Esc → 显示/隐藏 windowUi → 调用 GlobalGamePause 暂停/恢复。
/// 三个按钮：继续游戏（关闭菜单恢复游戏）、跳转 menu 2（黑幕过渡）、退出游戏。
/// </summary>
public class WindowLevel222 : MonoBehaviour
{
    [Header("UI 引用")]
    ...
    void Start()
    {
        ...
        if (menu2Button == null)
            menu2Button = GetButtonInChildren("Menu2Button");
        if (quitButton == null)
    ...
    /// <summary>跳转 menu 2：黑幕淡入 → 预加载 → 跳转。</summary>
    ...
    {
        Debug.Log("[WindowLevel222] QuitGame called — 退出游戏");
#if UNITY_EDITOR
    ...
    void BindButtons()
    {
        if (continueButton != null)
        {
            ...
            Debug.LogWarning("[WindowLevel222] continueButton 未赋值");

        if (menu2Button != null)
        {
            menu2Button.onClick.RemoveAllListeners();
            menu2Button.onClick.AddListener(GoToMenu2);
        }
        else
            Debug.LogWarning("[WindowLevel222] menu2Button 未赋值");
        ...
        if (quitButton != null)
        {
            ...
            Debug.LogWarning("[WindowLevel222] quitButton 未赋值");
    }
    ...
    void SetupRaycast()
    {
        ...
        if (canvas == null)
        {
            Debug.LogWarning("[WindowLevel222] 未找到 Canvas！");
            return;
        }
